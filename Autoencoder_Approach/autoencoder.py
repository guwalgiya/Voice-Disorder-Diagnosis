...
def main(input_vector_length, x_train, x_validate, arch_bundle, train_bundle):
    ...
    for i in range(encoder_layer):
        output_length  = encoding_dimension * (4 ** (encoder_layer - i))
    ...
    for i in range(encoder_layer):
        output_length  = encoding_dimension * (4 ** (i + 1))
    ...
    # =============================================================================
    early_stopping = EarlyStopping(monitor = 'val_loss', patience = 10, verbose = 0, min_delta = 0.001, mode = 'min')
