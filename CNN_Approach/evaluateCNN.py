...
def evaluateCNN(trained_model, test_combo, test_data, test_snippet_labels, test_augment_amount, classes):
    ...
    # ===============================================
    softmax_predictions = trained_model.predict(test_data)


    # ===============================================
    # Change softmax prediction to label
    for i in range(len(softmax_predictions)):
        max_weight_index = argmax(softmax_predictions[i])
        predicted_snippet_labels.append(classes[max_weight_index])
